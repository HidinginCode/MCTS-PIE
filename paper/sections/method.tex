\subsection{Problem definition: Pathfinding in Interactive Environments}
\label{sec:problem}
...
\begin{itemize}
    \item $f_1(\pi) = T$ -- the number of steps,
    \item $f_2(\pi) = \sum_{t=0}^{T-1} w(p_t)$ -- the total weight shifted,
    \item $f_3(\pi) = \mathrm{dist}(s_T, c_K)$ -- the residual Manhattan distance to the final checkpoint, used to penalize unsolved trajectories during search.
\end{itemize}
