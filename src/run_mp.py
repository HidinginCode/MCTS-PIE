...
def estimate_memory_per_worker_mb():
    return 600  # realistisch für deinen Code
...
def run_case(params):
    log_name = (
        f"mp_logs/mcts_{params['map']}_{params['env_dim']}_"
        f"{params['seed']}_{params['budget']}_{params['rollout']}.log"
    )
...
if __name__ == "__main__":
    ...
    processes = compute_safe_worker_count(args.processes, total_memory_mb)

    print(f"Using {processes} workers (memory-safe).")
